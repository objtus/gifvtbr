...
```
GET /variant/smile?unset    # 適用中なら解除、未適用なら何もしない
```

stackMode との組み合わせ:
- stackMode OFF: 適用中なら `setVariant(-1)`（ベースにリセット）
- stackMode ON: スタックに含まれていれば取り除く、含まれていなければ何もしない

> `?set` と `?unset` を同時に指定した場合、`?set` が優先される。
